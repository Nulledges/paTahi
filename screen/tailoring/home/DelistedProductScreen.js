...
const DelistedProductScreen = props => {
  ...
  const getStore = state => state.store.myStore;
  const getUserStoreProducts = state => state.products.userStoreProducts;

  const getInactiveUserProducts = createSelector(
    [getUserStoreProducts],
    userStoreProducts =>
      userStoreProducts.filter(product => product.isActive === false),
  );
  const userProduct = useSelector(getInactiveUserProducts);

  const userStoreId = useSelector(getStore);
  ...
  const renderItem = ({item}) => (
    <MyProductItems
      ...
      onPressEdit={() => {
        props.navigation.navigate('EDIT PRODUCT', {productId: item.id});
      }}
    />
  );
  return (
    <View style={styles.container}>
      {userProduct == '' && (
        <View style={styles.cardContainer}>
          <Text style={styles.textStyle}>No delisted products</Text>
        </View>
      )}
      {userProduct != '' && (
        <FlatList
