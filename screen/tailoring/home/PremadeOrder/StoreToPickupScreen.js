...
const StoreToPickupScreen = props => {
  const dispatch = useDispatch();
  const storeInformation = useSelector(state => state.store.myStore);
  const storeToPickupOrders = useSelector(
    state => state.order.storeToPickupItems,
  );
  useEffect(() => {
    dispatch(orderActions.fetchStorePickupOrders(storeInformation.storeId));
    console.log('StoreToPickupScreen');
  }, []);
  /*  useFocusEffect(
    useCallback(() => {
      dispatch(orderActions.fetchStorePickupOrders(storeInformation.storeId));
  ...
  const renderItem = ({item}) => {
    return (
      <View>
        <TouchableOpacity
          onPress={() => {
            dispatch(orderActions.updateStorePremadeToPickupSeen(item.id));
        ...
        <View style={styles.finishedContainer}>
          <MainButton
            ...
            onPress={() => {
              dispatch(orderActions.updateToPickupOrder(item.id, item.userId));
            }}
          />
        </View>
      </View>
...
export default StoreToPickupScreen;
