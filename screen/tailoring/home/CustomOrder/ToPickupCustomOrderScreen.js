...
const ToPickupCustomOrderScreen = props => {
  const dispatch = useDispatch();
  const storeInformation = useSelector(state => state.store.myStore);
  const toPickupOrderRequest = useSelector(
    state => state.order.storeToPickupRequest,
  );

  useEffect(() => {
    dispatch(orderActions.fetchToPickupOrder(storeInformation.storeId));
  }, []);
        ...
        <View
          ...
          <View>
            <MainButton
              ...
              onPress={() => {
                dispatch(orderActions.updateStoreToPickupSeen(item.id));
                props.navigation.navigate('STORE PICKUP CUSTOM ORDER DETAIL', {
                  orderItems: item,
                });
              }}
            />
...
export default ToPickupCustomOrderScreen;
