...
const LiveProductScreen = props => {
  ...
  const getActiveUserProducts = createSelector(
    [getUserStoreProducts],
    userStoreProducts =>
      userStoreProducts.filter(product => product.isActive === true),
  );
  const userProduct = useSelector(getActiveUserProducts);

  const getStore = useSelector(state => state.store.myStore);
  ...
  useEffect(() => {
    try {
      const unsubcribe = dispatch(
        productActions.fetchUserStoreProducts(getStore.storeId),
      );
      return unsubcribe;
    } catch (error) {
