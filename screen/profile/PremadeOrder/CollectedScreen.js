...
const CollectedScreen = props => {
  ...
  const collectedOrders = useSelector(state => state.order.userCollectedItems);
  /*  useFocusEffect(
    useCallback(() => {
      return dispatch(orderActions.fetchUserCollectedPremadeOrders);
    }, [dispatch]),
  ); */

  useEffect(() => {
    dispatch(orderActions.fetchUserCollectedPremadeOrders);
    console.log('CollectedScreen');
  }, []);
    ...
    return (
      <View>
        ...
        <View>
          <TouchableWithoutFeedback
            onPress={() => {
              dispatch(orderActions.updateUserPremadeCollectedSeen(item.id));
