...
const RefundedScreen = props => {
  const dispatch = useDispatch();
  const refundedOrders = useSelector(state => state.order.userRefundedItems);
  /*  useFocusEffect(
    useCallback(() => {
      return dispatch(orderActions.fetchUserRefundedPremadeOrders);
    }, []),
  ); */
  useEffect(() => {
    dispatch(orderActions.fetchUserRefundedPremadeOrders);
    console.log('RefundedScreen');
  }, []);
  ...
  const renderItem = ({item}) => {
    return (
      <View>
        ...
        <TouchableOpacity
          onPress={() => {
            dispatch(orderActions.updateUserPremadeRefundedSeen(item.id));
