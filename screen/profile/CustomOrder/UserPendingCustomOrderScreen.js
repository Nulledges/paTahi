...
const UserPendingCustomOrderScreen = props => {
  ...
  useEffect(() => {
    dispatch(orderActions.fetchUserPendingOrder);
    console.log('UserPendingCustomOrderScreen');
  }, []);
  const renderItem = ({item}) => {
    /*   const milliseconds =
      item.dateOrdered.seconds * 1000 +
      Math.floor(item.dateOrdered.nanoseconds / 1e6);
        ...
        <View
          ...
          <View>
            <MainButton
              ...
              onPress={() => {
                dispatch(orderActions.updateUserPendingSeen(item.id));
                props.navigation.navigate('USER PENDING CUSTOM ORDERS DETAIL', {
                  orderItems: item,
                });
              }}
            />
...
export default UserPendingCustomOrderScreen;
